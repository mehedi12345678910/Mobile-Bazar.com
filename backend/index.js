  ...
  try {
    ...
    app.get("/my-orders", verifyJWT, async (req, res) => {
      try {
        ...
        console.error(err);
        res.status(500).send({ message: "Failed to load my orders" });
      }
    });

    app.get("/manage-orders", verifyJWT, verifySELLER, async (req, res) => {
      const email = req.tokenEmail; // JWT থেকে email
      const result = await ordersCollection
    ...
    app.delete(
      ...
      async (req, res) => {
        const id = req.params.id;
        const result = await sellerRequestCollection.deleteOne({
          _id: new ObjectId(id),
        });
        res.send(result);
      },
    ...
    app.delete("/mobiles/:id", verifyJWT, verifySELLER, async (req, res) => {
      const id = req.params.id;
      const result = await mobileCollection.deleteOne({
        _id: new ObjectId(id),
      });
      res.send(result);
    });
